 ...
 *
 */
public class UnionDFUtils {

	/**
	 * 代付
	 * demo代码copy
	 * 参考文档：https://open.unionpay.com/ajweb/product/detail?id=67
	 * @param orderId 自定义规则为 yyyyMMddHHmmss+bankcardID
	 * @param total_fee
	 * @param cardNO 银行卡号
	 * @param name 姓名
	 * @return
	 ...
	 */
	public static String doDF(String orderId,double total_fee,String cardNO,String name){
		Map<String, String> contentData = new HashMap<String, String>();
		
		/***银联全渠道系统，产品参数，除了encoding自行选择外其他不需修改***/
		contentData.put("version", Payment.UNIONPAY.VERSION);               //版本号
		...
		//String accNo = AcpService.encryptData(cardNO, Payment.UNION.INPUT_CHARSET); //这里测试的时候使用的是测试卡号，正式环境请使用真实卡号
		//contentData.put("accNo", accNo);
		//////////
		
		/////////商户未开通敏感信息加密的权限那么不对敏感信息加密使用：
		contentData.put("accNo", "cardNO");                  //这里测试的时候使用的是测试卡号，正式环境请使用真实卡号
		////////
...
//		customerInfoMap.put("certifId", "341126197709218366");		    //证件号码
		...
		String customerInfoStr = AcpService.getCustomerInfo(customerInfoMap,cardNO,Payment.UNIONPAY.INPUT_CHARSET);				
		
		contentData.put("customerInfo", customerInfoStr);
		contentData.put("txnAmt", CommonUtils.formatMoney("0",total_fee * 100));						 		//交易金额 单位为分，不能带小数点
		...
		contentData.put("backUrl", "http://192.168.254.55/9tong-zml-app/paynotify/union/3");
		...
		Map<String, String> rspData = AcpService.post(reqData,requestBackUrl,Payment.UNIONPAY.INPUT_CHARSET);        //发送请求报文并接受同步应答（默认连接超时时间30秒，读取返回结果超时时间30秒）;这里调用signData之后，调用submitUrl之前不能对submitFromData中的键值对做任何修改，如果修改会导致验签不通过
