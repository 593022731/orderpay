 ...
 * 
 * @author : weihui
 * @createTime : 2016年5月23日 上午10:36:25
 * @version : 1.0
 * @description :
 *
 */
public class UnifiedorderPayReqDateH5 {
	 ...
	 *            微信支付分配的商户号[是]
	 * @param device_info
	 *            终端设备号(门店号或收银设备ID)，注意：PC网页或公众号内支付请传"WEB"[否]
	 * @param body
	 *            商品或支付单简要描述[是]
	 * @param detail
	 *            商品名称明细列表[否]
	 ...
	 * @param out_trade_no
	 *            商户系统内部的订单号,32个字符内、可包含字母, 其他说明见商户订单号[是]
	 * @param fee_type
	 *            符合ISO 4217标准的三位字母代码，默认人民币：CNY，其他值列表详见货币类型[否]
	 * @param total_fee
	 *            订单总金额，只能为整数，详见支付金额[是]
	 * @param spbill_create_ip
	 *            APP和网页支付提交用户端ip，Native支付填调用微信支付API的机器IP。[是]
	 * @param time_start
	 ...
	 * @param goods_tag
	 *            商品标记，代金券或立减优惠功能的参数，说明详见代金券或立减优惠[否]
	 * @param notify_url
	 *            接收微信支付异步通知回调地址[是]
	 * @param trade_type
	 *            取值如下：JSAPI，NATIVE，APP，WAP,详细说明见参数规定[是]
	 * @param product_id
	 *            trade_type=NATIVE，此参数必传。此id为二维码中包含的商品ID，商户自行定义。[否]
	 * @param openid
	 *            trade_type=JSAPI，此参数必传，用户在商户appid下的唯一标识。下单前需要调用【网页授权获取用户信息】
	 *            接口获取到用户的Openid。[否]
	 */
	public UnifiedorderPayReqDateH5(String appid, String mch_id,
		...
		// 根据API给的签名规则进行签名
		String sign = Signature.getSignH5(toMap());
